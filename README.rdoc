= Scanty, a really small blog, now on the CouchDB
...
This Branch is using the Makura CouchDB Ruby wrapper
...
I like bundled gems, so this scanty includes the merb bundler.
Not sure if bin/thor will work for you so you may need to install the 
thor gem manually then use plain thor instead of bin/thor.
...
Then run the server:
  ...
  $ bin/thin start -p 4567 -R config/rackup-dev.ru
...
Gentoo has CouchDB 0.8.1 in portage now. However this scanty won't work with 0.8.1 so you need to install the latest stable release 0.9.0.
A guide is available at http://nicholasorr.com/blog/how-to/install-couchdb-on-gentoo
...
Works with CouchDB 0.9.0 & Makura 2009.03.28
